...
class Api::V1::Peers::SearchController < Api::BaseController
  ...
  def set_domains
    ...
    if Chewy.enabled?
      ...
      @domains = Instance.searchable.where(Instance.arel_table[:domain].matches("#{Instance.sanitize_sql_like(domain)}%", false, true)).limit(10).pluck(:domain) if domain.length > 2
    end
  end
end
